...
plt.subplot(3,1,1)
plt.title('chi2')
plt.pcolormesh(periodCorn, dOmegaCorn, chi2, vmax=chiMaxPlt, rasterized=True)
plt.axis(pltCorners)
plt.colorbar()

plt.subplot(3,1,2)
plt.title('entropy')
plt.pcolormesh(periodCorn, dOmegaCorn, entropy, vmin=2.0*np.max(entropy), rasterized=True)
plt.axis(pltCorners)
plt.colorbar()

plt.subplot(3,1,3)
plt.title('test')
plt.pcolormesh(periodCorn, dOmegaCorn, test, rasterized=True)
plt.axis(pltCorners)
plt.colorbar()
...
print('for contours around the chi^2 minumum at a confidence level of {:}: '.format(targetProb))
...
plt.axis(pltCorners)
plt.xlabel('Period (d)')
plt.ylabel('dOmega (rad/d)')
ax.ticklabel_format(useOffset=False)
plt.colorbar()

ax = plt.subplot(2,1,2)
plt.title('realative prob.')
plt.pcolormesh(periodCorn, dOmegaCorn, probOfDiff, rasterized=True)
plt.axis(pltCorners)
plt.xlabel('Period (d)')
plt.ylabel('dOmega (rad/d)')
ax.ticklabel_format(useOffset=False)
...
#plt.subplot(3,1,3)
#plt.title('abs. prob.')
#plt.pcolormesh(periodCorn, dOmegaCorn, probArray, rasterized=True)
#plt.axis(pltCorners)
#plt.colorbar()
